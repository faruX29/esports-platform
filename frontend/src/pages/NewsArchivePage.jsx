  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', margin: '10px 0 6px' }}>
        <span style={{ fontSize: 10, color: '#f4f4f4', fontWeight: 800, textTransform: 'uppercase', letterSpacing: 1, padding: '3px 7px', borderRadius: 999, background: `${visuals.gameColor}22`, border: `1px solid ${visuals.gameColor}55` }}>
          {typeTag(row)}
        </span>
        <span style={{ fontSize: 10, color: '#d8d8d8', padding: '3px 7px', borderRadius: 999, background: 'rgba(255,255,255,.04)', border: '1px solid #272727' }}>
          {visuals.gameLabel}
        </span>
        <span style={{ fontSize: 10, color: '#a5a5a5', padding: '3px 7px', borderRadius: 999, background: 'rgba(255,255,255,.03)', border: '1px solid #242424' }}>
          Tier {visuals.tier}
        </span>
      </div>
      <h3 style={{ margin: '0 0 6px', fontSize: 16, lineHeight: 1.3 }}>{row.title}</h3>
      {row.summary && (
        <p style={{ margin: 0, color: '#a9a9a9', fontSize: 13, lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
          {row.summary}
        </p>
      )}
      <div style={{ marginTop: 8, fontSize: 11, color: '#7f7f7f' }}>{fmtDate(row.created_at)}</div>
    </Link>
  )
}
    ...
    <div style={{ minHeight: '100vh', background: '#090909', color: '#f2f2f2' }}>
      ...
      <div style={{ maxWidth: 1180, margin: '0 auto', padding: isMobile ? '14px 10px 34px' : '22px 16px 48px' }}>
        <div style={{ marginBottom: 16 }}>
          ...
          <p style={{ margin: 0, color: '#9b9b9b', fontSize: isMobile ? 13 : 14 }}>
            ...
            {GAME_TABS.map(tab => {
              ...
              return (
                <button key={tab.id} onClick={() => setParam({ oyun: tab.id, sayfa: 1 })}
                  style={{ padding: '7px 13px', borderRadius: 999, border: active ? '1px solid #C8102E' : '1px solid #2a2a2a', background: active ? 'rgba(200,16,46,.18)' : '#121212', color: active ? '#fff' : '#9e9e9e', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                  {tab.label}
                </button>
              )
            })}
            ...
            {TYPE_TABS.map(tab => {
              ...
              return (
                <button key={tab.id} onClick={() => setParam({ tur: tab.id, sayfa: 1 })}
                  style={{ padding: '6px 12px', borderRadius: 999, border: active ? '1px solid #5eead4' : '1px solid #2a2a2a', background: active ? 'rgba(94,234,212,.14)' : '#121212', color: active ? '#ddfffb' : '#9e9e9e', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>
                  {tab.label}
                </button>
              )
            })}
        ...
        {loading && <div style={{ color: '#888', fontSize: 13 }}>Arşiv yükleniyor...</div>}

        {!loading && rows.length === 0 && (
          <div style={{ color: '#777', fontSize: 13 }}>Bu filtre için haber bulunamadı.</div>
        )}
          ...
          <div style={{ marginTop: 20, display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            {page > 1 ? (
              <Link to={`?${new URLSearchParams({ ...(game !== 'all' && { oyun: game }), ...(type !== 'all' && { tur: type }), ...(page - 1 > 1 && { sayfa: page - 1 }) }).toString()}`}
                style={{ border: '1px solid #303030', background: '#181818', color: '#ddd', borderRadius: 8, padding: '6px 12px', textDecoration: 'none' }}>
                ‹ Önceki
              </Link>
            ) : (
              <span style={{ border: '1px solid #222', background: '#111', color: '#666', borderRadius: 8, padding: '6px 12px' }}>‹ Önceki</span>
            )}
            ...
            {page < totalPages ? (
              ...
              <span style={{ border: '1px solid #222', background: '#111', color: '#666', borderRadius: 8, padding: '6px 12px' }}>Sonraki ›</span>
            )}
          </div>
        )}
      </div>
