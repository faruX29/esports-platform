...
export default function ThemeToggle() {
  ...
  // Giden ikon aşağı + saatin tersine; gelen ikon yukarıdan düz iner.
  const iconStyle = shown => ({
    position: 'absolute',
    inset: 0,
    transform: shown ? 'translateY(0) rotate(0deg) scale(1)' : 'translateY(150%) rotate(-70deg) scale(.6)',
    opacity: shown ? 1 : 0,
    transition: 'transform .5s cubic-bezier(.34,1.3,.5,1), opacity .34s ease',
  })
  ...
  return (
    ...
    >
      ...
      <span style={{ position: 'relative', width: 17, height: 17, overflow: 'hidden', display: 'block' }}>
        {/* Ay (koyuya geçmeyi temsil eder) */}
        <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" style={iconStyle(isLight)}>
