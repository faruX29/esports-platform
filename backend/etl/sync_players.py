...
class PlayerStatsSyncer:
    ...
    # ── 3) Aktif Maç Kadrolarını Yenile ───────────────────────────────────────

    def sync_all_active_rosters(self, days=90, force=False):
        """
        Son `days` gün içinde maçı olan tüm takımların kadrolarını yeniler.
        force=False → zaten yüklü olanları atla.
        """
        self.ensure_schema()
        ...
        print(f"👤 {len(teams)} takım için kadro çekiliyor "
              f"(son {days} gün, force={force})...")
        processed = 0
        upserted  = 0
        skipped   = 0
        ...
        with Database.get_connection() as conn:
            for idx, (team_id, team_name) in enumerate(teams, 1):
                ...
                delay = 0.2 if (idx % 50 != 0) else 2.0
                time.sleep(delay)

        print(f"\n📊 Aktif kadro sync: processed={processed} "
